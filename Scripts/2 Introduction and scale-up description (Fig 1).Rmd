...
- Processed/Ineq_merged_with_vaccine_data_plus_priority_groups.csv: data inequalities OWID/WHO vaccine data + metrics + priority groups
- Processed/Ineq_merged_with_vaccine_priority_groups.csv: priority groups metrics

- Fig 4. Vaccine coverage in high-risk populations (health care workers and older adults) and overall. 
- Supplementary Fig. 1. Percentage of countries using different vaccine products by income status.
- Supplementary Fig. 7. Map of countries by per capita Gross National Income quartile.
- Supplementary Table 1. Vaccine introduction and scale-up milestones by income status.
- Supplementary Table 2. Vaccine coverage in high-risk populations and overall according to WHO/UNICEF database.
- Supplementary Table 3.  Age thresholds for defining older adults.
...
```{r}
#OWID data country, milestone achievement date
...
# Merge data inequalities/OWID vac data / WHO vaccine data and calculate minimum vaccine campaign date (OWID vs WHO)
...
#Number countries with date in OWID or WHO
table(Ineq_vaccine_coverage$owid_who)

# Countries with date in both 
table(!is.na(
  Ineq_vaccine_coverage[Ineq_vaccine_coverage$owid_who != "no date", "diff_owid_who_intro"]
))

# WHO/OWID date difference
Ineq_vaccine_coverage %>%
  summarise(
    Q1     = quantile(diff_owid_who_intro, 0.25, na.rm = TRUE),
    median = median(diff_owid_who_intro, na.rm = TRUE),
    Q3     = quantile(diff_owid_who_intro, 0.75, na.rm = TRUE),
...
```{r}
# + Vaccine Introduction and scale-up metrics data (for figures)
...
```{r owid vacc plat}
# + Vaccine platforms (OWID) by GNIpc
...
# Save output: data inequalities OWID/WHO vaccine data + metrics + platforms
...
# Save output: data inequalities OWID/WHO vaccine data + metrics + priority groups
write.csv(Ineq_vaccine_coverage, "Data/Processed/Ineq_merged_with_vaccine_data_plus_priority_groups.csv")
```


```{r}
#### Boxplot milestone December 2021/2023 priority
collated = rbind(
...
#Figure 4
# 1) Labels: one count per quartile x date x group
labels_df <- collated %>%
  filter(!is.na(coverage)) %>%
  mutate(q_GNI_per_c_WB = factor(q_GNI_per_c_WB, levels = c(1,2,3,4))) %>%
  group_by(q_GNI_per_c_WB, date, group) %>%
  summarise(n_points = n(), .groups = "drop")

# Set upper limit at 1 for plotting
collated$coverage[collated$coverage>1] = 1 

# 2) Plot
F4_priority_groups <- ggplot(collated) +
  ...
  geom_point(
    ...
    x = 110,           # place just beyond the 100% tick
    hjust = 0,
    size = 3,
    colour = "grey40", 
    inherit.aes = FALSE
...
# Info for manuscript
n_by_gni_date_group <- collated %>%
  filter(!is.na(coverage)) %>%
  mutate(q_GNI_per_c_WB = factor(q_GNI_per_c_WB, levels = c(1,2,3,4))) %>%
  group_by(q_GNI_per_c_WB, date, group) %>%
  summarise(n_countries = n_distinct(Country.Name), .groups = "drop") %>%
  complete(q_GNI_per_c_WB, date, group, fill = list(n_countries = 0)) %>%
  arrange(group, date, q_GNI_per_c_WB)
...
for_sup_table_risk_pop <- Ineq_vaccine_coverage %>%
  select(
    ...
    perc_HCW, perc_old, 
    `cov_hcw_cps_2023-12-01`:`pol_age_old_source_2021-12-01`
  )
