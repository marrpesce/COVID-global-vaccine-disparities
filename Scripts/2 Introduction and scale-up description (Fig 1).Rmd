...
Ineq_vaccine_coverage <- Ineq_merged %>%
  ...
  mutate(
    ...
    mutate(
    diff_min_vac_intro_1Dec2020 = as.numeric(min_vac_intro - dmy("1-12-2020")), #1-12-2020 month were it was approved first COVID-19 vaccine for emergency supply
  ...
  mutate(
    diff_min_vac_intro_1Dec2020_clean=case_when(
      diff_min_vac_intro_1Dec2020<0 ~ 0,
      TRUE ~ diff_min_vac_intro_1Dec2020
    )
  ) %>%
...
summary_vacc_intro_scale <- Ineq_vaccine_coverage %>%
  ...
  summarize(
    ...
    median_value_1 = round(median(diff_min_vac_intro_1Dec2020, na.rm=TRUE),0),
    q25_1 = round(quantile(diff_min_vac_intro_1Dec2020, 0.25, na.rm=TRUE),0),
    q75_1 = round(quantile(diff_min_vac_intro_1Dec2020, 0.75, na.rm=TRUE),0),
...
#First vaccine introduction after 01-12-2020 by quartile

min_intro <- Ineq_vaccine_coverage %>%
  group_by(q_GNI_per_c_WB) %>%
  summarize(
min_vac_intro=min(as.Date(min_vac_intro)[!is.na(min_vac_intro) & as.Date(min_vac_intro) > as.Date("2020-12-01")])
  )
...
Ineq_vaccine_coverage <- Ineq_vaccine_coverage %>% 
  mutate(
            V1_group= case_when(
      diff_min_vac_intro_1Dec2020 < 30  ~ "<30",
      diff_min_vac_intro_1Dec2020 > 29 & diff_min_vac_intro_1Dec2020 < 60 ~ "30-59",
      diff_min_vac_intro_1Dec2020 > 59 & diff_min_vac_intro_1Dec2020 < 90 ~ "60-89",
      diff_min_vac_intro_1Dec2020 > 89 ~ "90+",
      TRUE ~ "not reached"
  ),
            V40_group= case_when(
...
intro_scale_delay<- Ineq_vaccine_coverage %>%
  select(Country.Name, q_GNI_per_c_WB, diff_min_vac_intro_1Dec2020, diff_intro_F40) %>%
  filter(!is.na(diff_min_vac_intro_1Dec2020)) %>%
  pivot_longer(
    cols = c(diff_min_vac_intro_1Dec2020, diff_intro_F40),
    names_to = "Diff_dates",
    values_to = "Value"
  ) 
...
c2_plot_intro_scale_delay <- ggplot(intro_scale_delay) +
  ...
  facet_wrap(~Diff_dates, scales = "free_x", labeller = labeller(Diff_dates = c("diff_min_vac_intro_1Dec2020" = "Time to vaccine introduction", "diff_intro_F40" = "Time to vaccine scale-up (40%)")))+
  labs(title = "(C)",
       x = "Number of days", 
       y = "GNI quartile")  +
  guides(color = "none") +
...
coverage_high_2021_2023<- WHO_hcw_old_data %>% 
  ...
  filter(date=="2021-12-01" | date=="2023-12-01") %>% 
...
```{r}
# HCW: Bar plot introduction/scale-up yes no per GNI per capita quartile

hcw_milestone_achieved <- Ineq_vaccine_coverage %>% 
  filter(!is.na(vac_intro_hcw)) %>% 
  group_by(q_GNI_per_c_WB) %>% 
  summarise(
    V1 = sum(V1_hcw_dummy == "yes"),
    V40 = sum(V40_hcw_dummy == "yes"),
    V70 = sum(V70_hcw_dummy == "yes"),
    total = n(),
    V1_per = round(sum(V1_hcw_dummy == "yes") / n() * 100, 1),
    V40_per = round(sum(V40_hcw_dummy == "yes") / V1 * 100, 1),
    V70_per = round(sum(V70_hcw_dummy == "yes") / V1 * 100, 1),
  )

hcw_milestone_achieved

hcw_milestone_achieved <- hcw_milestone_achieved %>% 
    pivot_longer(
    cols = c(V1_per, V40_per, V70_per),
    names_to = "milestone",
    values_to = "Value"
  )
...
plot_1a<- ggplot(hcw_milestone_achieved, aes(x = as.factor(q_GNI_per_c_WB), y = Value, fill = as.factor(q_GNI_per_c_WB))) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = c("#77AADD","#44BB99","#AAAA00","#EE8866")) +
  coord_flip()+
  facet_wrap(~ milestone, labeller = labeller(milestone = c("V1_per" = "Vaccine introduction (31/12/2021)", "V40_per" = "Vaccine scale-up (40%,31/12/2021)"))) +
  labs(title = "helath care workers",
       x = "",
       y = "% countries achieving milestone") +
    guides(fill = "none") +  # Remove the fill legend
  theme_minimal() +
...
# old: Bar plot introduction/scale-up yes no per GNI per capita quartile

old_milestone_achieved <- Ineq_vaccine_coverage %>% 
  filter(!is.na(vac_intro_old)) %>% 
  group_by(q_GNI_per_c_WB) %>% 
  summarise(
    V1 = sum(V1_old_dummy == "yes"),
    V40 = sum(V40_old_dummy == "yes"),
    V70 = sum(V70_old_dummy == "yes"),
    total = n(),
    V1_per = round(sum(V1_old_dummy == "yes") / n() * 100, 1),
    V40_per = round(sum(V40_old_dummy == "yes") / V1 * 100, 1),
    V70_per = round(sum(V70_old_dummy == "yes") / V1 * 100, 1),
  )

old_milestone_achieved

old_milestone_achieved <- old_milestone_achieved %>% 
    pivot_longer(
    cols = c(V1_per, V40_per, V70_per),
    names_to = "milestone",
    values_to = "Value"
...
plot_1b<- ggplot(old_milestone_achieved, aes(x = as.factor(q_GNI_per_c_WB), y = Value, fill = as.factor(q_GNI_per_c_WB))) +
  geom_bar(stat = "identity", position = "dodge") +
  scale_fill_manual(values = c("#77AADD","#44BB99","#AAAA00","#EE8866")) +
  coord_flip()+
  facet_wrap(~ milestone, labeller = labeller(milestone = c("V1_per" = "Vaccine introduction (31/12/2021)", "V40_per" = "Vaccine scale-up (40%,31/12/2021)"))) +
  labs(title = "older population",
       x = "",
       y = "% countries achieving milestone") +
    guides(fill = "none") +  # Remove the fill legend
  theme_minimal() +
...
```{r}
#ii) Health care workers: Box plot introduction/scale-up yes no per GNI per capita quartile
hcw_intro_scale_delay<- Ineq_vaccine_coverage %>%
  select(Country.Name, q_GNI_per_c_WB, vac_intro_hcw , vac_F40_hcw, vac_F70_hcw) %>%
  filter(!is.na(vac_intro_hcw)) %>%
  pivot_longer(
    cols = c(vac_intro_hcw, vac_F40_hcw, vac_F70_hcw),
    names_to = "dates",
    values_to = "Value"
  ) 

# Set factor levels
hcw_intro_scale_delay$dates <- factor(hcw_intro_scale_delay$dates,
                                      levels = c("vac_intro_hcw", "vac_F40_hcw", "vac_F70_hcw"))


hcw_plot_intro_scale_delay <- ggplot(hcw_intro_scale_delay) +
  geom_boxplot(aes(x = ymd(Value), y = as.factor(q_GNI_per_c_WB)), outlier.shape = NA) +
   geom_point(
    aes(
     x = ymd(Value), 
     y = as.factor(q_GNI_per_c_WB),
  ...
  scale_color_manual(values = c("#77AADD","#44BB99","#AAAA00","#EE8866")) +
  facet_wrap(~dates, scales = "free_x", labeller = labeller(dates = c("vac_intro_hcw" = "Vaccine introduction", "vac_F40_hcw" = "Scale-up (40%)", "vac_F70_hcw" = "Scale-up (70%)")))+
...
```{r}
# Older people: Box plot introduction/scale-up yes no per GNI per capita quartile
old_intro_scale_delay<- Ineq_vaccine_coverage %>%
  select(Country.Name, q_GNI_per_c_WB, vac_intro_old , vac_F40_old, vac_F70_old) %>%
  filter(!is.na(vac_intro_old)) %>%
  pivot_longer(
    cols = c(vac_intro_old, vac_F40_old, vac_F70_old),
    names_to = "dates",
    values_to = "Value"
  ) 

# Set factor levels
old_intro_scale_delay$dates <- factor(old_intro_scale_delay$dates,
                                      levels = c("vac_intro_old", "vac_F40_old", "vac_F70_old"))


old_plot_intro_scale_delay <- ggplot(old_intro_scale_delay) +
  geom_boxplot(aes(x = ymd(Value), y = as.factor(q_GNI_per_c_WB)), outlier.shape = NA) +
   geom_point(
    aes(
     x = ymd(Value), 
  ...
  #scale_x_continuous(limits = c(0, NA))+
  scale_color_manual(values = c("#77AADD","#44BB99","#AAAA00","#EE8866")) +
  facet_wrap(~dates, scales = "free_x", labeller = labeller(dates = c("vac_intro_old" = "Vaccine introduction", "vac_F40_old" = "Scale-up (40%)", "vac_F70_old" = "Scale-up (70%)")))+
  labs(title = "older population",
       x = "date", 
       y = "GNI quartile")  +
  guides(color = "none") +
...
old_plot_intro_scale_delay 

 old_plot_intro_scale_delay + hcw_plot_intro_scale_delay  

```



#Supplementary Table 2. Vaccine scale-up milestones in high-risk groups by income status.
