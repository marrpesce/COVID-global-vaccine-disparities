...
scatter_milestone_country <- Ineq_vaccine_coverage %>%
  select(Country.Name, GNI_per_c_WB, q_GNI_per_c_WB, min_vac_intro, owid_vac_F40) %>%
  mutate(
    owid_vac_F40 = as.Date(owid_vac_F40)
  ) %>% 
  pivot_longer(
...
ED_F0_scatterplot_GNI_milestone <- ggplot(scatter_milestone_country) +
  ...
  facet_wrap(~ milestone, 
             scales = "free", 
             labeller = labeller(milestone = c("min_vac_intro" = "Vaccine introduction", "owid_vac_F40" = "Vaccine scale-up (40%)" ))) + 
  scale_y_date(
