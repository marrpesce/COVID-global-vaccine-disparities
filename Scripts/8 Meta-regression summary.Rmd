...
# labels
axis_labels <- c("1 (lower\nincome)", "2" ,"3","4 (higher\nincome)")
VEmetareg$variant_clean = fct_relevel(VEmetareg$variant_clean, c("delta", "omicron"))
metareg_collated$variant_clean = metareg_collated$VoI
metareg_collated$variant_clean = fct_relevel(metareg_collated$variant_clean, c("delta", "omicron"))

# Plot
ggplot() +
  geom_point(
    data = metareg_collated,
    aes(x = GNI, y = pred, color = as.factor(GNI), shape = `N studies`),
    size = 2.5
