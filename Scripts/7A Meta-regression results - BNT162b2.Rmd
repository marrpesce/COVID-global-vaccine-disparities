...
# select vaccine
selected_vaccine = "BioNTech/Pfizer BNT162b2" # values: "BioNTech/Pfizer BNT162b2" "Oxford/AstraZeneca ChAdOx1-S" "Moderna mRNA-1273"
simple_name = "BNT162b2" # for file outputs 
fup_sensitivity = FALSE # set to true to run sensitivity
```
...
#number studies
# vaccine product
table_vacc <- VEmetareg_data_clean %>%
  ...
  distinct()
# GNI
table_GNI1 <- VEmetareg_data_clean %>%
...
Sum_delta_severe.GNI <- cbind.data.frame(pred.delta_severe.GNI, I_o_p_delta_severe.GNI) %>%
  ...
  mutate(RR_summary = 
         ...
         VE = (1-pred)*100,
         VE_lowerCI = (1-ci.ub)*100,
         VE_upperCI = (1-ci.lb)*100
         )

# Recalculate negative VE estimates
for (i in 1:nrow(Sum_delta_severe.GNI)) {
  if ( Sum_delta_severe.GNI$VE_lowerCI[i] <0 ) {
    ci.ub = Sum_delta_severe.GNI$ci.ub[i]
    ci.ub_inv = 1/ci.ub
    Sum_delta_severe.GNI$VE_lowerCI[i] = -(1-ci.ub_inv)*100
  }
}

# Calculate VE summary
Sum_delta_severe.GNI = Sum_delta_severe.GNI %>% mutate(
  VE_summary = 
           paste0(
             format(round(VE,0),nsmall=0)," (",
             format(round(VE_lowerCI,0),nsmall=0),"-",
             format(round(VE_upperCI,0),nsmall=0),")"
             )
) %>% select(-c(VE, VE_lowerCI, VE_upperCI))
...
# Recalculate negative VE estimates
for (i in 1:nrow(Sum_omicron_severe.GNI)) {
  if ( Sum_omicron_severe.GNI$VE_lowerCI[i] <0 ) {
    ci.ub = Sum_omicron_severe.GNI$ci.ub[i]
    ci.ub_inv = 1/ci.ub
    Sum_omicron_severe.GNI$VE_lowerCI[i] = -(1-ci.ub_inv)*100
  }
}

# Calculate VE summary
Sum_omicron_severe.GNI = Sum_omicron_severe.GNI %>% mutate(
  VE_summary = 
           paste0(
             format(round(VE,0),nsmall=0)," (",
             format(round(VE_lowerCI,0),nsmall=0),"-",
...
forestplot(
  ...
  is.summary = c(
    TRUE, TRUE,
    !is.na(tabletext[3:nrow(tabletext), 1]) & 
    (tabletext[3:nrow(tabletext), 1] %in% c("Summary", "GNI quartile 3", "GNI quartile 4"))
  ),
  zero = NA,
  xlog = TRUE,
  xticks = log(c(0.01, 0.1, 0.2, 0.5, 1)), 
