...
ED_F0_scatterplot_GNI_milestone <- ggplot(scatter_milestone_country) +
  ...
  facet_wrap(~ milestone, 
             scales = "free", 
             labeller = labeller(milestone = c("min_vac_intro" = "Vaccine introduction", "owid_vac_F40" = "Vaccine scale-up (40%)" , "min_VE_pub_date"= "1st VE research publication"))) + 
  scale_y_date(
...
spearman_corr <- scatter_milestone_country %>%
  ...
  summarise(
    ...
    p_value = round(cor.test(log(GNI_per_c_WB), as.numeric(date), method = "spearman")$p.value, 3)
  )
...
ineq_intro_scale_VE <- Ineq_merged %>%
  select(
    ...
    year_GNI, q_GNI_per_c_WB, `Income group`, min_vac_intro, owid_who, owid_vac_F40, 
    owid_vac_F70, vaccines, mRNA:Protein, min_VE_pub_date, n_VE_studies
  ) %>% 
  rename(
    min_vac_intro_owid_or_who = owid_who
  )

curated_dataset_ineq_intro_scale_VE_prior_group<- left_join(ineq_intro_scale_VE, Ineq_merged_prior_group)
