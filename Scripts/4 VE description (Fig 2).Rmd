...
## Fig xxx. Global inequities in the evaluation of vaccine effectiveness. (A) Map of the distribution of available VE studies across different countries. 
...
#Plot
plot2_e<- ggplot(plat_summary[plat_summary$vac_type != "Protein subunit", ], aes(x = vac_type, y =per_papers_vac_type , fill = vac_type)) +
  geom_bar(stat = "identity", position = "dodge") +
  coord_flip()+
  labs(title = "(E)",
       x = "",
...
```{r}
#Need to run the figures from script 2
# Option A:
map_3 <- map1_a / map1_b / fig2_map
map_3

Fig_2a <- (Fig_2_a_vac_achiev / Fig_2_b_vac_delay / Fig_2_c_plat)
Fig_2b <- (plot2_c / plot2_d / plot2_e)

# Wrap both sides of the "|" in parentheses
Fig_2 <- (Fig_2a | Fig_2b) + plot_layout(widths = c(2, 1))
...
# 
# 
# Fig_3 <-  (plot2_d | plot2_b | plot2_c) /  (plot2_e | plot2_f) 
...
timeB_a<- data_F3[data_F3$vac_type != "Protein subunit", ] %>%
...
timeB_b <- ggplot(paper_sem_quart[paper_sem_quart$vac_type != "Protein subunit", ], aes(x = semester, y = total_papers, fill = as.factor(vac_type))) +
  geom_bar(stat = "identity") +
